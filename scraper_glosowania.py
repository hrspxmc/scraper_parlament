...
adress_voting = (
    "https://www.sejm.gov.pl/Sejm9.nsf/agent.xsp?symbol=posglos&NrKadencji=9"
)
...
voting_id = namedtuple("voting_id", ["nr_kadencji", "nr_posiedzenia", "nr_glosowania"])

votes_list = [
    voting_id(
        int(re.search("NrKadencji=(.+?)&", ii).group(1)),
        int(re.search("NrPosiedzenia=(.+?)&", ii).group(1)),
        int(re.search("NrGlosowania=(.+)", ii).group(1)),
    )
    for ii in all_votings_pages
]
...
while remaining_keys:
    ...
    for ii in tqdm(chunks):
        try:
            output = parallel(
                delayed(scf.all_votings)(scf.vote_id_to_link(vote_id)) for vote_id in ii
            )
...
for key, value in voting_results.items():
    party_aggregated_votes[key] = {
        k: dict(Counter(v.values())) for k, v in value.items()
    }
