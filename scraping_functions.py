...
def get_project_for_eu(row_cells):
    ue_image = row_cells[0].find(
        "img", alt="Projekt zawierający przepisy wykonujące prawo UE"
    )
...
def assemble_voting_dict(votings_row_cells):
    return {
        "nr_pos_sejmu": unidecode(votings_row_cells[0].text),
...
def vote_id_to_link(vote_id):
    return "agent.xsp?symbol=glosowania&NrKadencji={}&NrPosiedzenia={}&NrGlosowania={}".format(
        vote_id.nr_kadencji, vote_id.nr_posiedzenia, vote_id.nr_glosowania
    )
...
def extract_personal_votes(club_voting_page_adress):
    ...
    return dict(zip([ii.text for ii in all_td[0::2]], [ii.text for ii in all_td[1::2]]))
...
def all_votings(voting_page_adress):
    ...
    clubs_voting_pages = {
        ii.text: ii.find("a")["href"] for ii in soup.find_all("td", {"class": "left"})
    }
    personal_votes_dict = {
        k: extract_personal_votes(v) for k, v in clubs_voting_pages.items()
    }
